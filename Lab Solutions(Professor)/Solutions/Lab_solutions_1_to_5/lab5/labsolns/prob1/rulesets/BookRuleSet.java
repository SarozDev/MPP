...
public class BookRuleSet implements RuleSet {
	...
	private void isbnRule() throws RuleException {
		...
		if(len == 10 && !(val.charAt(0) == '0' || val.charAt(0) == '1')) {
			throw new RuleException("First digit of 10-digit ISBN must be "
					+ "0 or 1");
		}
		if(len == 13 && !(val.startsWith("978") || val.startsWith("979"))) {
			throw new RuleException("All 13-digit ISBNs must begin with "
					+ "either '978' or '979'");
		}
	}
	private void priceRules() throws RuleException {
		...
		if(len < 3 || val.charAt(len-3) != '.') {
			throw new RuleException("Price must have exactly two digits after "
					+ "the decimal point.");
		}
		if(doubleVal <= 0.49) {
