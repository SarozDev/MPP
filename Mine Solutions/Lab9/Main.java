...
public class Main {
	...
	public static void ordering1(List<Integer> list) {
		...
		System.out.println(list.stream().sorted((a,b)->Math.abs(a) == Math.abs(b) ? Integer.compare(a,b) : Integer.compare(Math.abs(a), Math.abs(b))).collect(Collectors.toList()));
	...
	public static void ordering2(List<String> words) {
		...
		System.out.println(words.stream().sorted(Comparator.comparing(s->new StringBuilder(s).reverse().toString())).collect(Collectors.toList()));
